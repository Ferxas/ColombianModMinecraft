...
public class MCreatorLaSelvaDeLasFarc extends Elementsshitposting_colombian.ModElement {
	@ObjectHolder("shitposting_colombian:laselvadelasfarc")
	public static final ModDimension dimension = null;
	@ObjectHolder("shitposting_colombian:laselvadelasfarc")
	public static final Item block = null;
	@ObjectHolder("shitposting_colombian:laselvadelasfarc_portal")
	public static final CustomPortalBlock portal = null;
	public static DimensionType type = null;
	private static Biome[] dimensionBiomes;

	public MCreatorLaSelvaDeLasFarc(Elementsshitposting_colombian instance) {
		super(instance, 4);
	...
	public void registerDimension(RegistryEvent.Register<ModDimension> event) {
		event.getRegistry().register(new CustomModDimension().setRegistryName("laselvadelasfarc"));
	}

	@SubscribeEvent
	public void onRegisterDimensionsEvent(RegisterDimensionsEvent event) {
		if (DimensionType.byName(new ResourceLocation("shitposting_colombian:laselvadelasfarc")) == null) {
			DimensionManager.registerDimension(new ResourceLocation("shitposting_colombian:laselvadelasfarc"), dimension, null, true);
		}
		type = DimensionType.byName(new ResourceLocation("shitposting_colombian:laselvadelasfarc"));
	}

	@Override
	public void init(FMLCommonSetupEvent event) {
		dimensionBiomes = new Biome[]{ForgeRegistries.BIOMES.getValue(new ResourceLocation("swamp")),
				ForgeRegistries.BIOMES.getValue(new ResourceLocation("plains")), ForgeRegistries.BIOMES.getValue(new ResourceLocation("jungle")),
	...
	public void initElements() {
		...
		elements.items.add(() -> new ModTrigger().setRegistryName("laselvadelasfarc"));
	}
	...
	public static class CustomPortalBlock extends NetherPortalBlock {
		public CustomPortalBlock() {
			...
			setRegistryName("laselvadelasfarc_portal");
		}
		...
		public void animateTick(BlockState state, World world, BlockPos pos, Random random) {
			...
			if (random.nextInt(110) == 0)
				world.playSound(pos.getX() + 0.5, pos.getY() + 0.5, pos.getZ() + 0.5,
						(net.minecraft.util.SoundEvent) ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation(("shitposting_colombian:a_s"))),
			...
			protected int getDistanceUntilEdge(BlockPos p_180120_1_, Direction p_180120_2_) {
				...
				for (i = 0; i < 22; ++i) {
					...
					if (!this.func_196900_a(this.world.getBlockState(blockpos))
							|| !(this.world.getBlockState(blockpos.down()).getBlock() == MCreatorPenon.block.getDefaultState().getBlock())) {
						break;
					}
				}
				BlockPos framePos = p_180120_1_.offset(p_180120_2_, i);
				return (this.world.getBlockState(framePos).getBlock() == MCreatorPenon.block.getDefaultState().getBlock()) ? i : 0;
			}
				...
				label56 : for (this.height = 0; this.height < 21; ++this.height) {
					for (int i = 0; i < this.width; ++i) {
						...
						if (i == 0) {
							...
							if (!(this.world.getBlockState(framePos).getBlock() == MCreatorPenon.block.getDefaultState().getBlock())) {
								break label56;
							}
				...
				for (int j7 = -1; j7 <= 1; ++j7) {
					for (int i8 = 1; i8 < 3; ++i8) {
						for (int i9 = -1; i9 < 3; ++i9) {
							...
							this.world.setBlockState(blockpos$mutableblockpos, flag ? MCreatorPenon.block.getDefaultState().getBlock()
			...
			for (int k7 = -1; k7 < 3; ++k7) {
				for (int j8 = -1; j8 < 4; ++j8) {
					if (k7 == -1 || k7 == 2 || j8 == -1 || j8 == 3) {
						...
						this.world.setBlockState(blockpos$mutableblockpos, MCreatorPenon.block.getDefaultState().getBlock().getDefaultState(), 3);
					}
				}
			}
		...
		public BiomeProviderCustom(World world) {
			...
			for (Biome biome : this.biomes) {
				biome.addCarver(GenerationStage.Carving.AIR, Biome.createCarver(new CaveWorldCarver(ProbabilityConfig::deserialize, 256) {
					{
						carvableBlocks = ImmutableSet.of(Blocks.GRASS.getDefaultState().getBlock(), biome.getSurfaceBuilder().getConfig().getTop()
								.getBlock(), biome.getSurfaceBuilder().getConfig().getUnder().getBlock());
					}
